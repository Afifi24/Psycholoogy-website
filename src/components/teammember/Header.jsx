      ...
      <div className=' relative overflow-hidden  pt-4 lg:min-h-screen font-Manrope '>
         <nav className='flex items-center px-4 lg:px-10 justify-between '>
            ...
            <ul className=' hidden lg:flex  items-center gap-0'>
                ...
                <Link to={'/about'}><li onClick={()=>setTheme('about')} className={` p-[20px] text-lg hover:text-[#FFCBC7] duration-200 cursor-pointer ${theme === 'about'? 'text-red-300':''} `}>About</li></Link>
                <Link to={'/sessions'}> <li onClick={()=>setTheme('sessions')} className={` ${theme === 'sessions' ? 'text-red-300':''}  p-[20px] text-lg hover:text-[#FFCBC7] duration-200 cursor-pointer `}>Sessions</li></Link>
               <Link to={'/contact'}><li onClick={()=>setTheme('contact')} className={`${theme === 'contact' ? 'text-red-300':''}  p-[20px] text-lg hover:text-[#FFCBC7] duration-200 cursor-pointer `}>Contact Us</li></Link>
                ...
                <div className='flex group items-center gap-2'>
                 ...
                 <Link to={'/'}><p onClick={()=>setTheme('home1')} className={` ${theme === 'home1' ? 'text-red-300':''} cursor-pointer hover:text-red-500 duration-200`} href=""> Home1</p></Link>
                 </div>
                 <div className='flex group duration-200 items-center gap-2'>
                 <p className='w-0 duration-200 group-hover:w-5  rounded-full h-[2px] bg-red-500'></p>
                 <Link to={'/home2'}><p onClick={()=>setTheme('home2')} className={` ${theme === 'home2'? 'text-red-300':''} cursor-pointer hover:text-red-500 duration-200`} href=""> Home2</p></Link>
                 </div>
                 <div className='flex group duration-200 items-center gap-2'>
                 <p className='w-0 duration-200 group-hover:w-5  rounded-full h-[2px] bg-red-500'></p>
